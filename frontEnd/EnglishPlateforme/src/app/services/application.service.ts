...
export class ApplicationService {
  ...
  private gatewayApi = 'http://localhost:8090/api/applications';
  ...
  getAll(): Observable<Application[]> {
    return this.http.get<Application[]>(this.gatewayApi, { headers: this.getAuthHeaders() });
  }
...
changeStatusToAccepted(id: number, status: string, teacherEmail?: string, teacherPassword?: string) {
  let url = `${this.gatewayApi}/${id}/statusA?status=${status}`;
...
changeStatusToRejected(id: number, status: string, teacherEmail?: string, teacherPassword?: string) {
  let url = `${this.gatewayApi}/${id}/statusR?status=${status}`;
...
downloadCv(appId: number): Observable<Blob> {
  return this.http.get(`${this.gatewayApi}/download/${appId}`, {
